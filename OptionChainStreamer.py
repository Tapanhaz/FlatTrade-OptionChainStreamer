...
from others import FlatApiPy, check_symbols  
from datetime import datetime
from time import time
from flattrade import check_session_token
...
class OCStreamer(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def run_thread(self,thread_name, timeout=None):
        if thread_name == 'login':
            t = threading.Thread(target=self.FlattradeLogin, daemon=True)
            t.start()
            t.join(timeout)
        if thread_name == 'logout':
            t = threading.Thread(target=self.FlattradeLogout, daemon=True)
            t.start()
            t.join(timeout)
    ...
    def set_session(self, hard_refresh= False):
        self.update_login_label("Please wait.")

        user, pwd, token = check_session_token(hard_refresh= hard_refresh)
        ...
        if ret:
            res = self.api.get_userdetails()
            return res
        
    def FlattradeLogin(self):
        
        res = self.set_session()

        if res:
            if res["stat"] == "Ok":
                self.update_login_label(f"Welcome {res['uname'].title()}")
                self.static_token_initialize()
                self.wbhandler.start()
                self.subscribe()
            else:
                emsg = res.get("emsg", "Please check manually")
                self.update_login_label(f"{emsg} :: Retrying")

                res = self.set_session(hard_refresh=True)
                ...
                if res:
                    if res["stat"] == "Ok":
                        ...
                        emsg = res.get("emsg", "Please check manually")
                        self.update_login_label(f"{emsg}")
                else:
                    self.update_login_label("Error :: Please Enable Debug and Check")
        else:
            self.update_login_label("Error :: Please Enable Debug and Check")

    def FlattradeLogout(self): 
