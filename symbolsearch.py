...
class SearchScrip:   
    ...
    def get_symbols(self, 
        ...
        inst_url = f"https://pidata.flattrade.in/scripmaster/json/{exch.lower()}"
        try:           
            with httpx.Client(http2=True) as client:
                res = client.get(inst_url)
    ...
    def search_scrip(self,
                     exch: Literal['NSE','NFO', 'CDS','MCX']='NFO',
            ...
            if trading_symbol is None:
                ...
                for key, value in kwargs.items():
                    ...
                    if key == 'symbol':
                        ...
                        query_parts.append(f"{key} in ['{exp}']")
                ...
                if strike_price is not None:
                    query_parts.append(f"strike in [{strike_price}]")
    ...
    def get_strikediff(self,
        ...
        try:
            ...
            if symbol is not None:
                strike_list = list(set(sym_df.query(f"symbol in ['{symbol}']")['strike']))
                filtered_strikes = sorted([float(i) for i in strike_list if float(i) > 0])
    ...
    def get_tokendict(self,
        ...
        try:
            ...
            if token is not None:
                values = sym_df.query(f"token in ['{token}']")[['strike','optiontype']].iloc[0]
                return {str(token):{'strikeprice': values['strike'], 'optiontype': values['optionType']}}
            if tradingsymbol is not None:
                values = sym_df.query(f"tradingsymbol in ['{tradingsymbol}']")[['token','strike','optiontype']].iloc[0]
                return {values['token']: {'strikeprice': values['strike'], 'optiontype': values['optiontype']}}
        except Exception as e:
            logger.debug("Error fetching strikediff :: {}".format(e))
